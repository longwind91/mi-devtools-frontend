...
let dockSperateActionDelegateInstance;
export class DockSperateActionDelegate {
    static instance(opts = { forceNew: null }) {
        const { forceNew } = opts;
        if (!dockSperateActionDelegateInstance || forceNew) {
            dockSperateActionDelegateInstance = new DockSperateActionDelegate();
        }
        return dockSperateActionDelegateInstance;
    }
    handleAction(_context, _actionId) {
        if (route) {
            route.connectionInternal.sendRawMessage(JSON.stringify({ id: 1, method: 'Custom.oepnSperateDevtools', params: {} }));
        }
        return true;
    }
}
...
export class NodeIndicator {
    ...
    constructor() {
        ...
        const shadowRoot = UI.Utils.createShadowRootWithCoreStyles(element, { cssFile: 'entrypoints/inspector_main/nodeIcon.css', delegatesFocus: undefined });
