...
export class PersistenceImpl extends Common.ObjectWrapper.ObjectWrapper {
  ...
  async _moveBreakpoints(from, to) {
    const breakpoints = this._breakpointManager
      .breakpointLocationsForUISourceCode(from)
...
export class PathEncoder {
  ...
  encode(path) {
    return path
      .split('/')
      .map(token => this._encoder.toChar(token))
      .join('')
  }
  decode(path) {
    return path
      .split('')
      .map(token => this._encoder.fromChar(token))
      .join('/')
  }
}
